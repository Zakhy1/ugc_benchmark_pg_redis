...
async def benchmark_user_likes_list(
    ...
    if cache_time is not None:
        logger.info(
            f"[Бенчмарк] Список лайков пользователя (Кэш): {cache_time*1000:.2f} мс"
        )
...
async def benchmark_user_bookmarks_list(
    ...
    if cache_time is not None:
        logger.info(
            f"[Бенчмарк] Список закладок пользователя (Кэш): {cache_time*1000:.2f} мс"
        )
    return db_time, cache_time
...
async def benchmark_review_like_add_and_read(
    ...
    if cache_update_time is not None:
        logger.info(
            f"[Бенчмарк] Обновление кэша рецензии: {cache_update_time*1000:.2f} мс"
        )
    logger.info(
        f"[Бенчмарк] Чтение статистики рецензии после добавления (БД): {read_db_time*1000:.2f} мс"
    )
    if read_cache_time is not None:
        logger.info(
            f"[Бенчмарк] Чтение статистики рецензии после добавления (Кэш): {read_cache_time*1000:.2f} мс"
        )
